...
class LCMAgent():
    def __init__(self, cfg, se, command_profile):
        ...
        # Note: Hard-coding this to be 15 right now so that the policy will run
    ...
    def get_obs(self):
        ...
        if "LastActionSensor" in self.cfg["sensors"]["sensor_names"]:
            ob = np.concatenate((ob,
                            self.last_actions.cpu().detach().numpy().reshape(1, -1)), axis=1)

        # if self.cfg["env"]["observe_clock_inputs"]:
        if "ClockSensor" in self.cfg["sensors"]["sensor_names"]:
            ob = np.concatenate((ob,
        ...
        if "YawSensor" in self.cfg["sensors"]["sensor_names"]:
        ...
        if "FeetContactSensor" in self.cfg["sensors"]["sensor_names"]:
        ...
        # print("OBSERVATION SHAPE in lcm_agent.py: ", ob.shape)
    ...
    def step(self, actions, hard_reset=False):
        ...
        # clock accounting
        # print(obs.shape, self.commands.shape)
